# 에이전트 시스템 설계서: 학술 논문 자동 번역 및 Typst 조판 시스템 (V4)

## 1. 작업 컨텍스트 문서

* **배경 및 목적**: 영문 학술 논문 PDF를 한국어로 번역하고, Typst를 활용해 원문의 레이아웃(다단, 이미지, 표 등)을 충실히 재현한 한국어 논문 PDF로 자동 재조판한다. 개인 학습 목적으로 사용하며, 속도보다 번역의 자연스러움을 우선한다.
* **실행 환경**: **Cursor IDE (Agent/Composer)** 내부에서 네이티브로 실행. Cursor의 규칙(`.cursor/rules`) 및 스킬 체계를 활용한다.
* **대상 OS**: Windows (크로스 플랫폼 지원 불필요)
* **대상 논문**: 다양한 분야(CS, 의학, 물리 등), 다양한 레이아웃(1단/2단), 다양한 길이의 학술 논문
* **입력**: 영문 논문 원본 파일 (`.pdf`) — 한 번에 한 편씩 처리
* **출력**: 
  * 최종 한국어 번역본 (`.pdf` — Typst 컴파일 결과물)
  * Typst 소스 코드 파일 (`.typ`) 및 추출된 비텍스트 에셋(이미지 등) 폴더
  * 논문별 용어집 (`.json`) — 자동 생성
* **핵심 설계 원칙 (LLM vs 스크립트 분리)**:
  * **코드/스크립트**: 파일 I/O, PDF 파싱 및 크롭, CLI 명령어 실행 등 **결정론적이고 기계적인 작업**을 전담. 표, 블록 수식, 알고리즘/의사코드는 철저히 이미지로만 취급한다.
  * **에이전트(LLM)**: 번역, 문맥 기반 레이아웃 해석, 에러 로그 분석 후 코드 수정 등 **비정형 데이터 생성 및 추론**을 전담. Cursor 내장 모델(Gemini 3.1 Pro 등)을 사용한다.
...
```mermaid
flowchart TD
    A["Step 1: PDF 분석/구조 추출"] --> B["Step 2: 비텍스트 추출 및 캡션 분리"]
...
|---|---|---|
| **입출력/파싱** | PDF 파싱(PyMuPDF+GROBID), **표/그림/블록 수식/알고리즘의 강제 이미지 크롭** | 파싱 데이터의 논리적 문서 구조 추론 및 검증 |
| **용어 관리** | glossary.json 파일 I/O | 전문 용어 추출 및 번역어 결정 |
| **번역/검증** | 텍스트 청킹(섹션 기반) | 2패스 번역(초벌+윤문), 용어집 준수, **인라인 수식 보존, 표 내부 텍스트 무시** |
| **수식 변환** | 폰트 기반 수식 영역 식별 + 유니코드→Typst 기본 매핑 | 복잡한 수식의 Typst 마크업 교정 |
| **조판/컴파일** | `typst compile` CLI 실행 및 에러 캡처 | 텍스트와 에셋(이미지화된 표/수식/알고리즘 포함)을 배치하는 Typst 마크업 생성 |
| **유지보수** | — | Typst 컴파일 에러 원인 분석 및 코드 디버깅 |

### [에러 처리 전략]

| 단계 | 에러 유형 | 처리 방식 |
|---|---|---|
| Step 1 | PDF 파싱 실패 (암호화, 스캔 이미지 PDF 등) | 즉시 중단 + 사용자에게 원인 안내 |
| Step 2 | 비텍스트 크롭 좌표 오류 | 에이전트가 좌표 재조정 시도 → 실패 시 휴먼 리뷰에서 수동 지적 |
| Step 4 | 번역 품질 미달 (자기 검증 실패) | 해당 섹션 재번역 (최대 2회) → 실패 시 경고 표시 후 진행 |
| Step 5 | Typst 컴파일 에러 | 에러 로그 분석 → 소스 수정 → 재컴파일 (최대 3회) → **3회 실패 시 중단 + 사용자 알림** |

### [폴더 구조]

```text
/pdf-typst-translator
  ├── /.cursor
  │   └── /rules
  │       ├── main-agent.mdc              # 메인 에이전트 지침 (워크플로우 오케스트레이션)
  │       ├── translator.mdc              # 번역 에이전트 규칙 (8대 원칙 + 2패스 전략)
  │       └── typst-coder.mdc             # Typst 마크업 생성 및 에러 디버깅 규칙
  ├── /scripts
  ...
  ├── /input                              # 원본 PDF 파일
  ├── /output
  │   ├── /assets                         # [휴먼 리뷰 1] 크롭된 표, 그림, 블록 수식, 알고리즘 이미지
  │   ├── /src                            # main.typ 소스 및 포함 파일
  │   ├── glossary.json                   # 논문별 자동 생성 용어집
  │   ├── assets_manifest.json            # 에셋 메타데이터 (파일명, 유형, 캡션, 원본 위치)
  │   ├── progress.json                   # 작업 진행 상태 (재개용)
  │   └── final_translated.pdf            # [휴먼 리뷰 2] 최종 조판 산출물
